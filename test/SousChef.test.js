...
const MasterGamer = artifacts.require('MasterGamer');
const XpBar = artifacts.require('XpBar');
const Player2 = artifacts.require('Player2');
const MockBEP20 = artifacts.require('libs/MockBEP20');

contract('Player2', ([alice, bob, carol, dev, minter]) => {
  beforeEach(async () => {
    this.syrup = await MockBEP20.new('LPToken', 'LP1', '1000000', {
      from: minter,
    });
    this.chef = await Player2.new(this.syrup.address, '40', '300', '400', {
      from: minter,
    });
  });
  ...
  it('try syrup', async () => {
    ...
    this.syrup = await XpBar.new(this.cake.address, { from: minter });
    this.lp1 = await MockBEP20.new('LPToken', 'LP1', '1000000', {
      from: minter,
    });
    this.chef = await MasterGamer.new(
      this.cake.address,
      this.syrup.address,
      dev,
      '1000',
    ...
    this.chef2 = await Player2.new(this.syrup.address, '40', '600', '800', {
      from: minter,
    });
